...
// Get topicid if param is set for getting statistics
...
if (isset($topicid)) {
	...
	// Get subtopics with their statistics
	$subtopics = $REST->get("statistics", array('topicid'=>$topicid));
	...
	if ($subtopics) {
		...
		foreach ($subtopics as $subtopic) {
			...
			$graphic .= $subtopic->subtopicname.' ('.$subtopic->count_from.'/'.$subtopic->count_to.') - '.$subtopic->procent.' %';
...
$PAGE->set_url('/mod/term/statics.php', array('id' => $cm->id));
...
// Start output
echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('mapping_stats_heading', 'term'));
